...
<div class="rd-card mb-3"><div class="card-body">
  ...
  <p class="small text-muted mb-2"><?= (int) ($route['progress_completed'] ?? 0) ?> de <?= (int) ($route['progress_total'] ?? 0) ?> passos concluídos</p>
  ...
  <div class="list-group mb-3">
    <?php foreach ($tasks as $task): ?>
      <div class="list-group-item d-flex justify-content-between align-items-start">
        ...
        <span class="badge <?= (($task['status'] ?? 'pending') === 'completed') ? 'text-bg-success' : 'text-bg-secondary' ?>"><?= e((string) ($task['status'] ?? 'pending')) ?></span>
      </div>
    <?php endforeach; ?>
  </div>
  ...
  <?php else: ?>
    <div class="alert alert-light border py-2 mb-0">Conclui todas as tarefas para desbloquear a recompensa.</div>
  <?php endif; ?>
</div></div>

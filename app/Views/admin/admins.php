<?php $admins = $admins ?? []; $roles = $roles ?? []; $matrix = $permission_matrix ?? []; ?>
...
<p class="small text-muted mb-0 mt-2">Governança reforçada: emails únicos, password mínima e bloqueio de desactivação do último super admin.</p>
</div></div>

<div class="rd-card mb-3"><div class="card-body table-responsive">
  <h6 class="mb-2">Matriz de permissões por papel</h6>
  <table class="table table-modern align-middle mb-0">
    <thead><tr><th>Papel</th><th>Permissões</th></tr></thead>
    <tbody>
      <?php foreach ($matrix as $role => $permissions): ?>
      <tr>
        <td><strong><?= e((string) $role) ?></strong></td>
        <td><?php foreach ($permissions as $permission): ?><span class="rd-badge badge-active me-1"><?= e((string) $permission) ?></span><?php endforeach; ?></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
...
<thead><tr><th>ID</th><th>Nome</th><th>Email</th><th>Papel</th><th>Estado</th><th>Criado por</th><th>Actualizar</th></tr></thead>
<tbody>
<?php foreach ($admins as $admin): ?>
<tr>
  <td><?= (int) $admin['id'] ?></td>
  <td><?= e((string) $admin['name']) ?><br><small class="text-muted">Atualizado: <?= e((string) ($admin['updated_at'] ?? '—')) ?></small></td>
  <td><?= e((string) $admin['email']) ?></td>
  <td><?= e((string) $admin['role']) ?></td>
  <td><?= e((string) $admin['status']) ?></td>
  <td><?= e((string) ($admin['created_by_name'] ?? 'n/d')) ?></td>
  <td>

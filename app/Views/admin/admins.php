...
<p class="small text-muted mb-0 mt-2">Governança reforçada: validação de papel, proteção do último super admin, confirmação explícita para mudanças sensíveis e auditoria obrigatória.</p>
</div></div>
...
<thead><tr><th>ID</th><th>Nome</th><th>Email</th><th>Papel</th><th>Estado</th><th>Criado / Atualizado</th><th>Governança</th><th>Actualizar</th></tr></thead>
<tbody>
...
<tr>
  ...
  <td><small>Alterações auditadas: <strong><?= (int) ($admin['changes_count'] ?? 0) ?></strong></small></td>
  <td>
    <form method="post" action="/admin/admins/<?= (int) $admin['id'] ?>/update" class="d-flex flex-wrap gap-1" onsubmit="return confirm('Confirmar atualização administrativa?');"><?= csrf_field() ?>
      ...
      <input name="confirm_super_admin_change" placeholder="CONFIRMAR (se sensível)" class="form-control form-control-sm" style="width:180px">
...
<div class="rd-card mt-3"><div class="card-body table-responsive">
  ...
  <?php foreach ($history as $item): ?>
    <tr><td><?= e((string) ($item['created_at'] ?? '')) ?></td><td><?= e((string) ($item['actor_name'] ?? 'system')) ?></td><td><?= e((string) ($item['action'] ?? '')) ?></td><td>admin#<?= (int) ($item['target_id'] ?? 0) ?> · <?= e((string) ($item['target_name'] ?? 'n/d')) ?></td><td><code><?= e((string) ($item['metadata_json'] ?? '{}')) ?></code></td></tr>
  <?php endforeach; ?>
  </tbody></table>
</div></div>

    ...
    <div class="rd-card fade-in">
      <div class="card-body p-4 p-md-5">
        ...
        foreach (($provinces ?? []) as $province) {
            $provinceNames[(int) $province['id']] = (string) $province['name'];
        }
        ?>
          ...
          <div class="col-md-6">
            ...
            <select class="form-select" name="city_id" id="citySelect" required>
              ...
              <?php foreach (($cities ?? []) as $city): ?>
                <?php $provinceName = $provinceNames[(int) $city['province_id']] ?? 'Província'; ?>
                <option value="<?= (int) $city['id'] ?>" data-province="<?= (int) $city['province_id'] ?>"><?= e((string) $city['name'] . ' — ' . $provinceName) ?></option>
              <?php endforeach; ?>
            </select>
            <small class="text-muted">Escolha a cidade correta; a validação final é feita no servidor.</small>
          </div>

    ...
    <?php if (!empty($conversations)): foreach ($conversations as $conversation): ?>
      ...
      <a href="/messages?conversation=<?= (int) $conversation['id'] ?>" class="d-block border rounded p-2 mb-2 text-decoration-none <?= $isActive ? 'border-primary bg-primary-subtle shadow-sm' : 'border-light-subtle' ?>">
        ...
        <div class="small text-muted">
          <?php if (($conversation['last_message_type'] ?? 'text') === 'image'): ?>📷 Foto enviada<?php else: ?><?= e((string) ($conversation['last_message'] ?? 'Sem mensagens')) ?><?php endif; ?>
        </div>
        <div class="small text-muted">
          <?= e((string) ($conversation['last_message_at'] ?? $conversation['updated_at'])) ?>
          · <?= (int) ($conversation['other_online_status'] ?? 0) === 1 ? 'online' : 'offline' ?>
          <?php if (!empty($conversation['other_profile_photo'])): ?>· com foto de perfil<?php endif; ?>
        </div>
      </a>
  ...
  <div class="col-lg-8">
    <div class="rd-card"><div class="card-body">
      ...
      <?php if (!empty($context)): ?>
        ...
        <?php if (!empty($pg['has_more'])): ?><a class="btn btn-sm btn-outline-secondary mb-2" href="/messages?conversation=<?= $activeConversationId ?>&page=<?= (int) ($pg['page'] + 1) ?>">Carregar mensagens anteriores</a><?php endif; ?>
